...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($action === "create") {
    ...
    if ($name === "") {
      ...
      $msg = "Please choose one of your stations.";
    } else if ($start === "" || $end === "") {
      ...
      if (!mysqli_fetch_assoc($res)) {
        $msg = "You can only create collections from your own stations.";
      } else {
        $measurementIds = [];
        $stmt = mysqli_prepare($conn, "
  ...
  if ($action === "share") {
    ...
    if ($friendId <= 0) {
      ...
      if (!mysqli_fetch_assoc($res)) {
        $msg = "You can only share your own collection with one of your friends.";
      } else {
        $stmt = mysqli_prepare($conn, "
...
if ($selectedCollectionId > 0) {
  ...
  if ($selectedCollection) {
    ...
    $msg = "Collection not found or not shared with you.";
  }
}
  ...
  <div class="col-lg-4">
    <div class="card p-3">
      ...
      <?php if (count($stations) === 0): ?>
        <p class="empty-state">You need at least one station before creating a collection.</p>
      <?php else: ?>
        <form method="post">
    ...
    <?php if ($selectedCollection): ?>
      <div class="card p-3 mt-3">
        <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
          ...
          <a class="btn btn-sm btn-outline-secondary" href="/RPIF1/user/collections.php">Close</a>
        </div>
